...
export const SubjectBreakdown = ({ subjects }: { subjects: Array<{ "title": string, "pass_criteria": number, "color": ColorValue, "bg": ColorValue, "icon": MaterialIconName, "attended_no_classes": number, "total_no_classes": number }> }) => {
    return (
...
const InduvidualSubjectInfo = ({ subject }: { subject: { "title": string, "pass_criteria": number, "color": ColorValue, "bg": ColorValue, "icon": MaterialIconName, "attended_no_classes": number, "total_no_classes": number } }) => {
    ...
    return (
        <View className="w-full h-26 bg-[#FFFFFF] rounded-xl p-4 flex flex-col gap-3">
            <View className="flex flex-row gap-3">
                <View style={{width: 40, height: 40, borderRadius: 8, alignItems: "center", justifyContent: "center", backgroundColor: subject.bg}}>
                    <MaterialIcons name={subject.icon} size={24} color={subject.color} />
                </View>
                <View className="flex flex-col flex-1">
                    <AppText className="text-primary-text text-fix font-lexend-7 leading-[18px] text-[16px]">{subject.title}{"\u200B"}</AppText>
                ...
                { (subject.attended_no_classes / subject.total_no_classes) * 100 >= subject.pass_criteria ? 
